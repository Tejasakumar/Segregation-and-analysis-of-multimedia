			...
			.then((res) => {
				...
				if (res.data.photos) {
					const newFolderItems = res.data.photos.map(
						(photo) => `data:image;base64,${photo.reference_image}`
					);
					...
					res.data.photos.forEach((photo) => {
						if (!hashmap.has(photo._id.$oid)) {
							sethash((prevMap) => {
								...
								newMap.set(photo._id.$oid, `data:image;base64,${photo.reference_image}`);
								return newMap;
							});
						}
					});
