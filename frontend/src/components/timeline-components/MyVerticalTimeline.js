...
const MyVerticalTimeline = () => {
  ...
  const fetchImageDates = async (newPage) => {
    ...
    //   cont:len,
    // });
    // axios.post('http://localhost:4000/api/get-image-dates',{req })
    // .then((res)=>{
    //   console.log("res",res);
    ...
    try {
      ...
      setItems((prevItems) => {
        const newItems = data.filter(element => !prevItems.some(item => item.fileName === element.fileName));
        setLen(prevLen => prevLen + newItems.length);  
        return [...prevItems, ...newItems];
      });
    
      // setLen(prevLen => prevLen + data.length);
    ...
    // try {
    //   const response =  fetch('http://localhost:4000/api/get-image-dates', {
    //     method: 'POST',
    //     headers: { 'Content-Type': 'application/json' },
    //     body: JSON.stringify({
    //       page: newPage,
